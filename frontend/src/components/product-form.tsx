...
export function ProductFormDialog({ mode, product, onSuccess, trigger }: ProductFormDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // L-R3-2: mirror the migration-007 products CHECK lower bounds (> 0).
    // The DB enforces these as the last line of defense; reject early here
    // so the admin sees a clear message instead of a constraint error.
    if (price <= 0 || wholesalePrice <= 0) {
      toast.error("Retail and wholesale prices must be greater than 0")
      setSaving(false)
      return
    }
    ...
    if (!Number.isNaN(itemsPerLot) && itemsPerLot <= 0) {
      toast.error("Items per lot must be at least 1")
      setSaving(false)
      return
    }
    if (!Number.isNaN(minWholesaleLots) && minWholesaleLots <= 0) {
      toast.error("Min wholesale lots must be at least 1")
      setSaving(false)
      return
    }
          ...
          <div className="space-y-2">
            ...
            {specRows.length === 0 ? (
              <p className="rounded-sm border border-border bg-card px-3 py-2 text-xs text-muted-foreground">
                No specs yet. Click &ldquo;Add spec&rdquo; to add one.
              </p>
            ) : (
              <div className="space-y-2">
                {specRows.map((row) => (
